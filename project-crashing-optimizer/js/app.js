document.addEventListener('DOMContentLoaded', () => {
    ...
    async function captureReportEvent(title, specificLogMessageForThisEvent = null) {
        ...
        // The ganttSvgElement global variable might be stale if the SVG is re-created.
        // It's safer to query for the current SVG within the parent container.
        const ganttParentContainer = document.querySelector('.app-container .gantt-container');
        const currentGanttSvgElement = ganttParentContainer ? ganttParentContainer.querySelector('svg#ganttChartContainer') : null;

        if (!activities || activities.length === 0) {
            logMessage(`Skipping report event capture for "${title}": No activities loaded.`, "debug");
            // Still push an event, but mark Gantt as not included
            reportEvents.push({ /* ... summary data ... */ ganttImage: null, title, logEntry: specificLogMessageForThisEvent || "No activities" });
            return;
        }
        
        let ganttImage = null;
        // Check if the current SVG element exists and seems to have content
        if (currentGanttSvgElement && typeof html2canvas === 'function' && ganttParentContainer) {
            // This delay should now be more effective because drawGanttChart's promise makes sure
            // the new Gantt instance has at least started its rendering process.
            const renderDelay = 1200; // Increased delay slightly for good measure
            logMessage(`Waiting ${renderDelay}ms for Gantt to render before capture for event "${title}"...`, "debug");
            await new Promise(resolve => setTimeout(resolve, renderDelay));

            // Double check the SVG is still there and has some children (Frappe Gantt adds groups, rects, etc.)
            const liveGanttSvgForCapture = ganttParentContainer.querySelector('svg#ganttChartContainer'); // Re-query for the absolute latest

            if (liveGanttSvgForCapture && liveGanttSvgForCapture.children.length > 0 && liveGanttSvgForCapture.style.display !== 'none') {
                logMessage(`Capturing Gantt image for event: "${title}" (after ${renderDelay}ms delay). Target element has ${liveGanttSvgForCapture.children.length} children.`, "debug");
                try {
                    const canvas = await html2canvas(ganttParentContainer, { // Capture the PARENT container
                        scale: 1.5, 
                        logging: false, 
                        useCORS: true,
                        backgroundColor: getComputedStyle(ganttParentContainer).backgroundColor || '#0D1220', // Use actual bg color
                        // Ensure html2canvas captures the container that has definite dimensions
                        width: ganttParentContainer.scrollWidth, 
                        height: ganttParentContainer.scrollHeight,
                        x: 0, y: 0
                    });
                    ganttImage = canvas.toDataURL('image/png');
                    logMessage(`Gantt image captured successfully for "${title}"`, "debug");
                } catch (err) {
                    logMessage(`Error capturing Gantt image for "${title}": ${err.message}`, "error");
                    console.error(`html2canvas error for "${title}":`, err);
                    ganttImage = null; // Explicitly null on error
                }
            } else {
                logMessage(`Gantt container not visible, empty, or SVG not found for event "${title}" even after delay. Image not captured. SVG children: ${liveGanttSvgForCapture ? liveGanttSvgForCapture.children.length : 'N/A'}. Display: ${liveGanttSvgForCapture ? liveGanttSvgForCapture.style.display : 'N/A'}`, "debug");
                ganttImage = null;
            }
        } else {
            logMessage(`Skipping Gantt image capture: currentGanttSvgElement not found or html2canvas not available for "${title}".`, "debug");
            ganttImage = null;
        }
        ...
        reportEvents.push({
            ...
            ganttImage: ganttImage // This will be null if capture failed
        });
        logMessage(`Report event "${title}" added. Total events: ${reportEvents.length}. Gantt included: ${!!ganttImage}`, "info");
    }

    // In project-crashing-optimizer/js/app.js

// ... (ensure jsPDF and jsPDF-AutoTable are loaded via <script> tags in your HTML)
// const { jsPDF } = window.jspdf; // Already in your original app.js if using UMD
    ...
    async function handleExportReportPDF() {
        if (reportEvents.length === 0) {
            ...
            logMessage("PDF Export: No report events to process.", "info");
            return;
        }

        logMessage("Generating PDF report... This may take a moment.", "info");
        
        // --- User Feedback: Disable button and show loading state ---
        const originalButtonText = exportReportPDFBtn.textContent;
        exportReportPDFBtn.disabled = true;
        exportReportPDFBtn.textContent = "GENERATING...";
        // You could also show a more prominent visual spinner on the page here
        ...
        try {
            ...
            function ensureNewPageIfNeeded(neededHeight = 40) {
                if (yPos + neededHeight > pageHeight - margin) {
                    doc.addPage();
                    yPos = margin;
                    // Reset font styles for new page if necessary
                    doc.setFontSize(10); doc.setFont(undefined, 'normal'); doc.setTextColor(0,0,0);
                }
            }
            
            function forceNewPage() {
                if (yPos > margin + 10) { // Avoid adding a blank page if already at the top
                    doc.addPage();
                }
                yPos = margin;
                doc.setFontSize(10); doc.setFont(undefined, 'normal'); doc.setTextColor(0,0,0);
            ...
            for (let i = 0; i < reportEvents.length; i++) {
                ...
                logMessage(`PDF: Processing event ${i + 1}/${reportEvents.length}: "${event.title}"`, "debug");

                if (i > 0) { // Start subsequent events on a new page
                    forceNewPage();
                } else { // For the first event, check space after title
                    ensureNewPageIfNeeded(80); // Estimate space for first event's title + summary
                }
                ...
                doc.autoTable({
                    ...
                    theme: 'striped', 
                    headStyles: { fillColor: [75, 75, 75], textColor: 255, fontSize: 10, fontStyle: 'bold' },
                    styles: { fontSize: 9, cellPadding: 3 },
                    columnStyles: { 0: { fontStyle: 'bold', cellWidth: 200 } },
                    tableWidth: 'auto', 
                    margin: { left: margin, right: margin } // Ensure right margin is also applied
                });
                yPos = doc.lastAutoTable.finalY + 15;

                // Associated Log Entry for this event
                if (event.logEntry && event.logEntry.trim() && event.logEntry !== "N/A") {
                    ensureNewPageIfNeeded(30 + (event.logEntry.length / (contentWidth/5) * 10) ); 
                    ...
                    yPos += (splitLog.length * 10) + 10; // Adjust line height (10pt for 9pt font is common)
                    doc.setFont(undefined, 'normal');
                }

                // Gantt Chart Image for this event (if captured)
                if (event.ganttImage) {
                    const neededHeightForImage = 200 + 20; // Max height for image + bottom margin
                    ensureNewPageIfNeeded(neededHeightForImage);
                    
                    const imgMaxHeight = 200; // Max height constraint for the image in PDF
                    const imgEffectiveWidth = contentWidth; // Max width constraint
                    
                    try {
                        const imgProps = doc.getImageProperties(event.ganttImage);
                        const aspectRatio = imgProps.width / imgProps.height;
                        
                        let pdfImgWidth = imgEffectiveWidth;
                        let pdfImgHeight = pdfImgWidth / aspectRatio;

                        if (pdfImgHeight > imgMaxHeight) {
                            pdfImgHeight = imgMaxHeight;
                            pdfImgWidth = pdfImgHeight * aspectRatio;
                        }
                        // Center the image if its width is less than contentWidth
                        const imgX = margin + (contentWidth - pdfImgWidth) / 2; 
                        
                        doc.addImage(event.ganttImage, 'PNG', imgX, yPos, pdfImgWidth, pdfImgHeight);
                        yPos += pdfImgHeight + 20; // Space after image
                    } catch (e) {
                        logMessage(`Error adding Gantt image for "${event.title}" to PDF: ${e.message}`, "error");
                        console.error("PDF AddImage Error for event '" + event.title + "':", e);
                        ensureNewPageIfNeeded(20); // Space for error message
                        doc.setTextColor(255, 0, 0); // Red color for error
                        doc.text("Error embedding Gantt image for this step.", margin, yPos);
                        yPos += 20;
                        doc.setTextColor(0, 0, 0); // Reset color
                    }
                } else {
                     logMessage(`PDF: No Gantt image to add for event "${event.title}".`, "debug");
                }
            ...
            if (finalEvent && finalEvent.activities && finalEvent.activities.length > 0) {
                forceNewPage(); // Ensure it starts on a new page
                ...
                const activityTableHeaders = [["ID", "Name", "Curr Dur", "ES", "EF", "LS", "LF", "Slack", "Crashed", "Can Crash?", "Critical?"]];
                const activityTableBody = finalEvent.activities.map(act => {
                    const canCrashThisActivity = (typeof act.currentDuration === 'number' && 
                                            typeof act.crashDuration === 'number' &&
            ...
            if (logArea.value.trim()) {
                ...
                logLinesForPDF.forEach(line => {
                    ensureNewPageIfNeeded(10); // Check space for each line
                    doc.text(line, margin, yPos);
                    yPos += 10; // Approximate line height for 8pt font
                });
            }
            ...
            logMessage(`Error during PDF report generation: ${error.message}`, "error");
            console.error("PDF Generation Error:", error);
            alert("An error occurred while generating the PDF report. Check the console for details.");
        } finally {
            // --- Reset Button State ---
            exportReportPDFBtn.disabled = false;
            exportReportPDFBtn.textContent = originalButtonText;
            // Hide any prominent visual spinner here
        }
    }
    ...
    async function handleCalculateInitialSchedule() { // Already async
        try {
            reportEvents = [];
            logArea.value = "";
            logMessage("Calculating initial schedule...", "info");

            gatherInputsAndBuildGraph(); 

            if (activities.length === 0) { 
                initialDurationDisplay.textContent = 'N/A';
                initialCostDisplay.textContent = 'N/A';
                // Clear other displays if needed
                await drawGanttChart(); // Still draw (it will show "no activities" message)
                return; 
            }
            ...
            calculateCPM(); // This updates currentProjectDuration, currentTotalCost etc.
            ...
            // currentDurationDisplay and currentCostDisplay are updated by calculateCPM > updateUIDisplay

            await drawGanttChart(); // MODIFICATION: Await here

            const initialLogSummaryForReport = `Initial State - Duration: ${currentProjectDuration.toFixed(2)}, Cost: $${currentTotalCost.toFixed(2)}, Critical Path: ${activities.filter(act => act.isCritical).map(act => act.id).join(', ') || 'N/A'}`;
    ...
    async function handleCrashOneStep() { // Already async
        try {
            logMessage("Attempting to crash one step...", "info");
            ...
            const criticalCrashableActivities = activities.filter(act =>
                act.isCritical && act.canCrashFurther() && 
                act.costPerUnitCrash !== Infinity && act.costPerUnitCrash >= 0
            );

            if (criticalCrashableActivities.length === 0) {
                // ... (no crash reason logic as before) ...
                let noCrashReason = "No further cost-effective crashing can be performed on the critical path.";
                // ... (determine specific reason) ...
                logMessage(noCrashReason, "warning");
                alert(noCrashReason);
                return; 
            }
            ...
            if (activityToCrash.costPerUnitCrash === Infinity) { /* ... safeguard ... */ return; }
            ...
            calculateCPM(); // Recalculates CPM and updates UI display (except Gantt)

            await drawGanttChart(); // MODIFICATION: Await here

            const finalCrashLogMessage = `CRASHED: Activity "${activityToCrash.id} (${activityToCrash.name})" by 1 unit. Cost: +$${activityToCrash.costPerUnitCrash.toFixed(2)}. Old Dur: ${oldDurationOfCrashedActivity} -> New Dur: ${activityToCrash.currentDuration}. New Project Duration: ${currentProjectDuration.toFixed(2)}. New Total Cost: $${currentTotalCost.toFixed(2)}.`;
            logMessage(finalCrashLogMessage, "info"); 
...
// ... (other functions and variables) ...
    ...
    async function drawGanttChart() {
        return new Promise((resolve, reject) => {
            const ganttParentContainer = document.querySelector('.app-container .gantt-container');

            if (!ganttParentContainer) {
                logMessage("Gantt parent container (.app-container .gantt-container) not found in DOM.", "error");
                reject(new Error("Gantt parent container not found"));
                return;
            }

            // --- AGGRESSIVE CLEARING ---
            // 1. Clear Frappe Gantt instance if it exists
            if (ganttChartInstance) {
                try {
                    ganttChartInstance.clear(); // Frappe Gantt's internal cleanup
            ...
            // 2. Remove ALL children from the parent container to ensure a clean slate
            while (ganttParentContainer.firstChild) {
                ganttParentContainer.removeChild(ganttParentContainer.firstChild);
            }
            logMessage("All child elements removed from Gantt parent container.", "debug");
            // --- END AGGRESSIVE CLEARING ---


            if (!activities || activities.length === 0) {
                const p = document.createElement('p');
                p.textContent = 'No activities to display. Calculate schedule first.';
                p.className = 'gantt-empty-message';
                ganttParentContainer.appendChild(p);
                logMessage("No activities to display in Gantt.", "debug");
                resolve({ success: true, drawn: false, message: "No activities to draw" });
                return;
            }

            // Create and append the new SVG *after* clearing
            const newGanttSvgElement = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            newGanttSvgElement.setAttribute("id", "ganttChartContainer");
            ganttParentContainer.appendChild(newGanttSvgElement);
            logMessage("New Gantt SVG element created and appended.", "debug");
            ...
            const tasksForGantt = activities.map(act => {
                if (isNaN(act.ES) || isNaN(act.EF)) return null;
            ...
            if (tasksForGantt.length === 0 && activities.length > 0) {
                logMessage("All activities had invalid dates for Gantt display.", "warning");
                const p = document.createElement('p');
                p.textContent = 'Could not display tasks; check ES/EF values.';
                p.className = 'gantt-empty-message';
                ganttParentContainer.appendChild(p);
                newGanttSvgElement.style.display = 'none'; 
                resolve({ success: true, drawn: false, message: "No valid tasks to draw" });
                return;
            }
            
            if (tasksForGantt.length > 0) {
                // Force a reflow before initializing Gantt. This makes the browser acknowledge the new SVG's dimensions.
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                const _unused = newGanttSvgElement.offsetHeight; 

                setTimeout(() => { // Keep the setTimeout
                    requestAnimationFrame(() => { // Keep the requestAnimationFrame
                        try {
                            if (!newGanttSvgElement || !newGanttSvgElement.parentElement) {
                                 logMessage("Gantt SVG element was removed before initialization. Aborting.", "warning");
                                 reject(new Error("Gantt SVG element missing during init"));
                                 return;
                            }
                            newGanttSvgElement.style.display = 'block'; // Ensure visibility
                            
                            ganttChartInstance = new Gantt(newGanttSvgElement, tasksForGantt, {
                                header_height: 50, column_width: 30, step: 24, 
                                view_modes: ['Day', 'Week', 'Month'], bar_height: 20,
                                bar_corner_radius: 3, arrow_curve: 5, padding: 18, 
                                view_mode: 'Week', date_format: 'YYYY-MM-DD', language: 'en',
                                custom_popup_html: function (task) {
                                    const activity = activities.find(a => a.id === task.id);
                                    if (!activity) return `<div class="gantt-popup-wrapper"><h5>${task.name || task.id}</h5><p>Details not found.</p></div>`;
                                    return `<div class="gantt-popup-wrapper"><h5>${activity.name} (ID: ${activity.id})</h5><p>Duration: ${activity.currentDuration.toFixed(1)} u</p><p>ES: ${activity.ES.toFixed(1)}, EF: ${activity.EF.toFixed(1)}</p><p>LS: ${activity.LS.toFixed(1)}, LF: ${activity.LF.toFixed(1)}</p><p>Slack: ${activity.slack.toFixed(1)}</p><p>Critical: ${activity.isCritical ? '<strong>Yes</strong>' : 'No'}</p><hr style="margin: 6px 0; border-color: var(--border-color-medium); opacity: 0.5;"><p style="font-size:11px;">Start: ${task.start} | End: ${task.end}</p></div>`;
                                }
                            });
                            logMessage("Gantt chart drawn/updated successfully (rAF).", "info");
                            resolve({ success: true, drawn: true, message: "Gantt drawn" });
                        } catch(e) {
                            logMessage(`Error initializing/drawing Gantt chart (rAF): ${e.message}`, "error");
                            console.error("Gantt Init/Draw Error (rAF):", e);
                            if (newGanttSvgElement) {
                               newGanttSvgElement.innerHTML = `<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="var(--accent-cyber-orange)" font-family="var(--font-mono)" font-size="14px">Error rendering Gantt. Check console.</text>`;
                            }
                            reject(e);
                        }
                    });
                }, 100); // Keep a small delay, 100ms might be safer than 50ms if issues persist
            } else {
                logMessage("No valid tasks to draw in Gantt chart (final check).", "debug");
                if(newGanttSvgElement) newGanttSvgElement.style.display = 'none';
                resolve({ success: true, drawn: false, message: "No tasks to draw after filtering" });
            }
        });
    }
    ...
    resetBtn.addEventListener('click', resetAllData);
        // Redraw Gantt button should also await
    redrawGanttBtn.addEventListener('click', async () => { // Make it async
        if (activities && activities.length > 0) {
            logMessage("Redrawing Gantt chart by user request...", "info");
            await drawGanttChart(); // Await here
        } else {
            logMessage("No activity data to draw Gantt chart.", "info");
            // Optionally, explicitly clear/show empty message if drawGanttChart doesn't handle it when no activities
            const ganttParentContainer = document.querySelector('.app-container .gantt-container');
            if (ganttParentContainer) {
                const oldSvgElement = ganttParentContainer.querySelector('svg#ganttChartContainer');
                if (oldSvgElement) oldSvgElement.remove();
                const existingMsg = ganttParentContainer.querySelector('p.gantt-empty-message');
                if (existingMsg) existingMsg.remove();
                const p = document.createElement('p');
                p.textContent = 'No activity data to draw Gantt chart.';
                p.className = 'gantt-empty-message';
                ganttParentContainer.appendChild(p);
            }
        }
